...
import { DESIGN_W } from './characterCardLayout';
...
export const RosterCharacterCard: React.FC<RosterCharacterCardProps> = ({
  ...
  return (
    <div className="flex w-full min-w-0 justify-center">
      <div
        onClick={openGear}
        className={`group relative flex w-full min-w-0 flex-col overflow-hidden rounded-md border transition-all duration-200 cursor-pointer ${
          isSelected
            ? 'border-amber-500 shadow-[0_0_15px_rgba(217,119,6,0.2)] scale-[1.01]'
            : 'border-stone-850 hover:border-stone-700'
        }`}
        style={{ maxWidth: DESIGN_W }}
      >
        <span
